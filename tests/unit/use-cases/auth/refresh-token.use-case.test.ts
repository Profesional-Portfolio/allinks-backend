...
describe('RefreshTokenUseCase', () => {
  ...
  describe('execute', () => {
    const mockPayload = { id: 'user-123', email: 'test@test.com' };
    const mockTokens = { accessToken: 'new-access-token', refreshToken: 'new-refresh-token' };

    it('should refresh tokens successfully', async () => {
      mockTokenProvider.verifyRefreshToken.mockResolvedValue([undefined, mockPayload as any]);
      mockCacheService.getRefreshToken.mockResolvedValue('old-refresh-token');
      mockTokenProvider.generateTokenPair.mockResolvedValue([undefined, mockTokens]);
      mockCacheService.setRefreshToken.mockResolvedValue(undefined);

      const [error, tokens] = await refreshTokenUseCase.execute('old-refresh-token');

      expect(error).toBeUndefined();
      expect(tokens).toEqual(mockTokens);
      expect(mockCacheService.setRefreshToken).toHaveBeenCalledWith('user-123', mockTokens.refreshToken);
    });
